...
class MultiImgSymmetricExchange(BaseTransform):
    def __init__(self, prob):
        self.prob = prob
        assert prob >= 0 and prob <= 1

    def symmetric_exchange(self, results: dict) -> dict:
        results['img'][0], results['img'][1] = results['img'][1], results['img'][0]
        w, h = results['gt_seg_map'].shape
        for i in range(w):
            for j in range(h):
                if results['gt_seg_map'][i][j] == 0:
                    continue
                if results['gt_seg_map'][i][j] >= 1 and results['gt_seg_map'][i][j] <= 4:
                    results['gt_seg_map'][i][j] += 4
                if results['gt_seg_map'][i][j] >= 5 and results['gt_seg_map'][i][j] <= 8:
                    results['gt_seg_map'][i][j] -= 4
                else:
                    raise ValueError('The matrix element value is abnormal. The element value must be in the range of [0,8]!')
        return results
    
    def transform(self, results: dict) -> dict:
        results = self.symmetric_exchange(results)
        return results
    
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += 'prob:' 
        repr_str += str(self.prob)
        return repr_str
